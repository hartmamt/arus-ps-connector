...
var ArusPSConnector = {
   ...
   */
  'getProfile': function(requestParams, model, useCache = true) {

    let cachedProfile = getCached('profile');

    if (typeof useCache !== 'boolean') {
      return Promise.reject(new TypeError(`Expected ${useCache} to be a booloean`));
    ...
    return new Promise((resolve, reject) => {
      if (typeof params !== 'object') {
        reject(new TypeError(`Expected ${params} to be an object`));
   ...
   * @return {Promise} - returns a Promise of a serialized remote request response
   */
  'getPicture': function(requestParams, model, useCache = true) {

    let cachedPicture = getCached('picture');

    if (typeof useCache !== 'boolean') {
      return Promise.reject(new TypeError(`Expected ${useCache} to be a boolean`));
  ...
  'getSchedule': function(requestParams, model, payloadMode = 1, acadCareer = 'UGRD') {
    // TODO: check for either requestParams.send OR payloadMode and acadCareer
    let defaults = {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!params.send) {
        ...
        params.send = `<SSR_GET_ENROLLMENT_REQ><SCC_ENTITY_INST_ID></SCC_ENTITY_INST_ID><EMPLID></EMPLID><ACAD_CAREER>${acadCareer}</ACAD_CAREER><INSTITUTION>UCINN</INSTITUTION><STRM></STRM><SSR_ENRL_GET_MODE>${payloadMode}</SSR_ENRL_GET_MODE></SSR_GET_ENROLLMENT_REQ>`;
      }
