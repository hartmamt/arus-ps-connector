 ...
 */
var { Profile, Picture, ScheduleModels, NotificationModels, ServiceIndicator } = require('./models/Models.js');
...
class Serializer {
  ...
  /**
  * Serializes data into a `ServiceIndicator` Object
  *
  * @method serviceIndicators
  * @static
  * @param {Object} serviceIndicatorData
  * @return {Array<ServiceIndicator>} - returns an Array of `ServiceIndicator` objects
  */
  static serviceIndicators(serviceIndicatorData) {
    let serviceIndicators = [];

    serviceIndicators = _.map(serviceIndicatorData.SCC_GET_SERVICE_IND_RESP.SERVICE_INDICATOR, (serviceIndicator) => {
      let serviceIndicatorData = {
        emplid: serviceIndicator.EMPLID[0],
        dateTime: serviceIndicator.SRVC_IND_DTTM[0],
        activeDateTime: serviceIndicator.SRVC_IND_ACTIVE_DT[0],
        institution: serviceIndicator.INSTITUTION[0],
        institutionDesc: serviceIndicator.INSTITUTION_DESCR[0],
        serviceIndicatorCode: serviceIndicator.SRVC_IND_CD[0],
        serviceIndicatorCodeDesc: serviceIndicator.SRVC_IND_CD_DESCR[0],
        serviceIndicatorReason: serviceIndicator.SRVC_IND_REASON[0],
        serviceIndicatorActTerm: serviceIndicator.SRVC_IND_ACT_TERM[0],
        serviceIndicatorActTermDesc: serviceIndicator.SRVC_IND_ACT_TERM_DESCR[0],
        departmentId: serviceIndicator.DEPTID[0],
        departmentIdDesc: serviceIndicator.DEPTID_DESCR[0],
        amount: serviceIndicator.AMOUNT[0],
        currencyCode: serviceIndicator.CURRENCY_CD[0],
        currencyCodeDesc: serviceIndicator.CURRENCY_CD_LOVDescr[0],
        serviceIndicatorReasonDesc: serviceIndicator.SRVC_IND_REASON_DESCR[0],
        positiveServiceIndicator: serviceIndicator.POS_SRVC_INDICATOR[0]
      };

      return new ServiceIndicator(serviceIndicatorData);
    });
